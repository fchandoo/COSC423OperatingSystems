...
public class BoundedBuffer  {     
 ...
 // producer and consumer will call this to nap
   ...
   public synchronized void enter(String name, Widget item) {
      while (count == BUFFER_SIZE) {
         try {
        	 System.out.println("Warning: worker " +name+ " is waiting to put"+item+"<handled by> conveyer belt");
        	 wait();
         }
         catch (InterruptedException e) { }
      }
	...
	if (count == BUFFER_SIZE) // this is when the buffer is full 
   ...
   public synchronized Object remove(String name) {
	...
	if (count == 0)// if the buffer is empty
