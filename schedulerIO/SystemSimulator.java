...
public GanntChart getChart() {
	// TODO Auto-generated method stub
	return chart;
}


public void doIO(int msec, Job job) {
	IODevice io = new IODevice(job, msec,this);
	myScheduler.startIO();
	myScheduler.clearRunningJob();
	io.start();
	
	
	job.getMyCondition().signal();
	try {
		job.getMyCondition().await();
	}
	catch(InterruptedException ex) {
		ex.printStackTrace();
