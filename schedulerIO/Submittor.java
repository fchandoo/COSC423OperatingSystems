...
//    	burstDescription=st.nextToken();
    	 ...
    	 * 
    	 */
    	String [] space=jobDesc.split(" "); 
    	id = "JOB" + space[0]; 
    	delay = Integer.parseInt(space[1]); 
    	
    	for(int i=2; i<space.length;i++) {
    		burstDescription.push(Integer.parseInt(space[1]));
    	}
    	...
    	System.out.println(Thread.currentThread()+ "notify the ready queue"); 
