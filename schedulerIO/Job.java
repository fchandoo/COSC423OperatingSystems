...
class Job extends Thread {
   ...
   */
  public Job(LinkedList<Integer> burstDescription, SystemSimulator s, String name, JobWorkable workToDo) {
    // Initialize stuff
    myOS = s;
    myCondition = s.getSingleThreadMutex().newCondition();
    
    //burstTime = Integer.parseInt( burstDescriptor );
    burstTimes = new LinkedList<Integer>(); 
    work = workToDo;
...
/*	  myOS.getSingleThreadMutex().lock();
	  ...
	  while (System.currentTimeMillis()-startTime < burstTimes.peek()) {// Not yet exhausted my run-time
      ...
      doCPU(burstTimes.remove(), true);
      while(!burstTimes.isEmpty()) {  // While there remains an IO/CPU pair
           doIO(burstTimes.remove());
           doCPU(burstTimes.remove(), false);
      }
      System.out.println(this.getNameOf()+ " "+ this + "is exiting"); 
      myOS.exit();  //exit needs to signal the condition, and release the lock
      System.out.println("Passed the OS exit"); 
  }
  
  private void doIO(int msec) {
	// TODO Auto-generated method stub
	  myOS.doIO(msec, this);
	
}

private void doCPU(int msec, boolean intialCPU) { //ask regarding the previous run
	// TODO Auto-generated method stub
	...
	startTime = System.currentTimeMillis();
	  while (System.currentTimeMillis()-startTime < burstTimes.peek()) {// Not yet exhausted my run-time
		  ...
		  myOS.getChart().recordEvent(startTime, System.currentTimeMillis(), this.getNameOf()+"Initial CPU");
		  
	  }else {
		  myOS.getChart().recordEvent(startTime,System.currentTimeMillis(), this.getNameOf()+"CPU Burst before IO burst" );
		  
	  }
	  System.out.println(this + "Has finished the CPU Burst"); 
